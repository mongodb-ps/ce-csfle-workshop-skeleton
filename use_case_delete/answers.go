...
var (
        PETNAME = "valid-rabbit"
        MDB_PASSWORD = "passwordone"
)
...
func main() {
  ...
  json.Unmarshal([]byte(schemaMap), &testSchema)
        ...
        err = encryptedColl.FindOne(context.TODO(), bson.M{"name.firstName": firstname}).Decode(&findResult)// PUT CODE HERE TO RETRIEVE DOCUMENT
        fmt.Printf("%+v\n", findResult)

        trashDEK(clientEncryption, kmsProvider, keySpace, employeeDEK)// PUT CODE HERE TO DELETE DEK
        /*if err != nil {
                return err
        }*/
        ...
        err = encryptedColl.FindOne(context.TODO(), bson.M{"name.firstName": firstname}).Decode(&findResult) // PUT CODE HERE TO RETRIEVE DOCUMENT, REMEMBER TO HANDLE ERRORS!
        fmt.Printf("%+v\n", findResult)
