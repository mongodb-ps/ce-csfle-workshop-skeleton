...
func decryptManual(c *mongo.ClientEncryption, d primitive.Binary)(bson.RawValue, error) {
        out, err := c.Decrypt(context.TODO(), d) // PERFORM THE DECRYPTION HERE
        if err != nil {
...
func main() {
  ...
  payload := bson.M{
    ...
    "role": []string{"DEV"},
        ...
        encryptedName, err = encryptManual(clientEncryption, dek, "AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic", "Kuber")// put required variables here)
        if err != nil {
                fmt.Printf("ClientEncrypt error: %s\n", err)
                exitCode = 1
                return
